...
- Fiscal series administrative row-lock alignment 4D-2A completado con PASS local: `BillingSeriesApplicationService.update()`, reactivacion mediante `update(active=true)` y `deactivate()` adquieren desde el inicio el mismo `findByIdForUpdate()`/`PESSIMISTIC_WRITE` que usa `createFromSale()`. Las validaciones de identidad, serie activa y maximo emitido operan sobre estado fresco bloqueado; una emision confirmada no puede ser sobrescrita por un payload administrativo obsoleto y el orden se mantiene `serie -> escrituras dependientes`. `FiscalSeriesConcurrencyIntegrationTest` valida PostgreSQL real con sesiones independientes, `pg_blocking_pids`, colas recursivas de waiters, rollback y timeouts acotados: emision vs update/deactivate/reactivate, dos emisiones, update vs deactivate, rollback antes/despues de escrituras, contador confirmado, duplicados/reutilizacion y ausencia de deadlock. Validaciones: focal 4D-2A 8 PASS; conjunto billing/4D 170 PASS; `BillingApplicationServiceTest` 134 PASS; suite backend completa 611 PASS. 4D-2 no se declara cerrada: 4D-2B mantiene diferidos `@Version`/ETag/`If-Match` para formularios administrativos obsoletos y 4D-2C mantiene diferida la identidad fiscal por ambiente/full number. Sin migraciones, REST, frontend, MiFact real, cambios 4D-1B, `.env`, secretos, commit, push ni tag.
