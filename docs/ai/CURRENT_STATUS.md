...
- Cierre de polish POS/E2E documentado en `docs/qa/POS_SEARCH_FEEDBACK_E2E_QA.md`: feedback de búsqueda/resultados refinado, mensaje de sin stock con nombre/SKU, helper E2E alineado y runtime fresco en `127.0.0.1:4201`; build y E2E no-write en PASS.
- Cierre de polish POS cart density/hierarchy documentado en `docs/qa/POS_CART_DENSITY_HIERARCHY_QA.md`: carrito mas compacto, fila operativa `Cant./Dscto./SUBTOTAL/Quitar` alineada, modal `Carrito completo` alineado al panel y correccion del microdesfase vertical entre `SUBTOTAL` y `Quitar`; build y E2E no-write en PASS.
- Cierre de polish POS checkout modal UX documentado en `docs/qa/POS_CHECKOUT_MODAL_UX_QA.md`: modal `Cobrar venta` mas ancho y en 2 columnas, `Checkout de cobro` eliminado, validacion progresiva para Factura, contrato E2E actualizado a elementos operativos reales; build y E2E no-write en PASS.
...
- Fiscal production safety hardening completado: `BillingRuntimeSafetyPolicy` centraliza reglas runtime por ambiente; PROD bloquea `createFromSale()` antes de consumir correlativo si no existen provider fiscal real y firma XML real; PROD bloquea firma con signer no productivo y envio con provider no productivo; `MockElectronicBillingProviderAdapter` no devuelve `ACCEPTED` para documentos PROD; LOCAL/BETA mantienen simulacion controlada; `./mvnw test` backend completo OK (476 tests, 0 failures). No implementa SUNAT/PSE/OSE real, UBL SUNAT completo, firma digital real, CDR, PDF fiscal, QR, notas ni bajas. No se tocaron `.env`, secretos ni certificados reales.
- Fiscal secrets DB/API foundation completado: Flyway V21 agrega referencias fiscales (`certificate_secret_ref`, `certificate_password_secret_ref`, `provider_secret_ref`, `certificate_alias`, `secret_provider`) sin modificar migraciones V11/V16; API acepta refs y metadata segura, y `CompanyBillingProfileResponse` ya no expone `certificatePath` ni `certificatePassword`; backend no mapea ni persiste `certificatePassword` plano, limpia el campo legacy al guardar perfiles, rechaza `certificatePassword`/`certificatePath` directos en PROD y exige refs seguras para perfiles PROD activos; Angular Configuracion tributaria usa refs write-only y metadata segura. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (41 tests), `./mvnw test` OK (482 tests), `npm run build` frontend OK. No implementa secret manager real, resolver de secretos, firma digital real, PSE/OSE/SUNAT real, CDR, PDF/ticket fiscal, QR, notas ni bajas; no se tocaron `.env`, secretos ni certificados reales.
- Fiscal secret resolver mock/local completado: se agrego `FiscalSecretResolverPort` y `LocalFiscalSecretResolverAdapter` no productivo para resolver solo placeholders seguros en LOCAL/BETA (`LOCAL_NOOP_CERT`, `LOCAL_NOOP_CERT_PASSWORD`, `LOCAL_NOOP_PROVIDER`, `BETA_SANDBOX_REF`, `BETA_SANDBOX_CERT_PASSWORD`, `BETA_SANDBOX_PROVIDER`); el resolver no lee `.env`, archivos, certificados, passwords, keystores ni claves privadas, no devuelve secretos reales y falla siempre en PROD con mensaje claro. `BillingRuntimeSafetyPolicy` ahora exige provider, signer y resolver productivos para considerar PROD listo, por lo que la emision real sigue bloqueada. Tests focalizados: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (49 tests). No se toco frontend ni DB/Flyway en esta fase; no se implemento secret manager real, SUNAT/PSE/OSE, firma real, CDR, XML UBL SUNAT completo, PDF/ticket fiscal, QR, notas ni bajas.
- Fiscal PROD enforcement / secret manager readiness completado: se agregaron propiedades fiscales no sensibles (`billing.secrets.provider`, `billing.secrets.production-enabled`, `billing.electronic.provider`, `billing.signer.provider`) con defaults seguros LOCAL/MOCK/NOOP; `BillingFiscalStartupValidator` hace fail-fast si se activa `production-enabled=true` sin provider/signer/resolver productivos; `BillingRuntimeSafetyPolicy` exige readiness completa tambien para firma y resultados aceptados en PROD; perfiles PROD activos rechazan placeholders `LOCAL_*`/`BETA_*` y providers `LOCAL`/`MOCK`/`NOOP` sin exponer valores enviados. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (58 tests), `./mvnw test` OK (499 tests). No se toco frontend ni DB/Flyway; no se implemento secret manager real, SUNAT/PSE/OSE, firma real, CDR, XML UBL SUNAT completo, PDF/ticket fiscal, QR, notas ni bajas; no se tocaron `.env`, secretos ni certificados reales.
- Fiscal lifecycle state machine + idempotency base completado: `ElectronicDocumentLifecyclePolicy` centraliza transiciones validas (`DRAFT -> GENERATED -> SIGNED -> SENT -> ACCEPTED/REJECTED/ERROR`) y bloquea transiciones invalidas; `generateXml`, `sign` y `send` ahora usan lock pesimista por documento; `generateXml` y `sign` son idempotentes en `GENERATED`/`SIGNED` sin historial redundante ni sobrescritura posterior; `send` bloquea reenvio en `SENT` o estados finales sin llamar otra vez al provider; V22 agrega indice unico parcial no destructivo por `sale_id` activo con precheck para impedir mas de un comprobante activo por venta. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (67 tests), `./mvnw test` OK (508 tests). No se toco frontend; no se implemento retry avanzado, attempts, provider response estructurada, secret manager real, SUNAT/PSE/OSE, firma real, CDR, XML UBL SUNAT completo, PDF/ticket fiscal, QR, notas ni bajas; no se tocaron `.env`, secretos ni certificados reales.
- Fiscal attempts send audit base: V23 y el modelo de attempts se conservan; 4D-1B sustituyo el uso de `REQUIRES_NEW` en el camino de envio por participacion `MANDATORY` dentro de transacciones locales cortas de preparacion/finalizacion. `STARTED + SENT` se confirman juntos antes del provider y la finalizacion de documento, attempt, historial y evidencia se confirma junta despues. Se mantienen hashes SHA-256, sanitizacion, `simulated=true` en LOCAL/BETA y ausencia de retry automatico.
- Fiscal provider response mapping vigente: `ACCEPTED -> SUCCESS/ACCEPTED`; `OBSERVED -> SUCCESS/ACCEPTED + PROVIDER_OBSERVED`; `REJECTED -> FAILED/REJECTED`; `PENDING -> PENDING/SENT`; `TIMEOUT`, `UNAVAILABLE`, `COMMUNICATION_ERROR` y excepciones ambiguas -> attempt `PENDING`, documento `SENT`, `recoverable=false`; `CONFIGURATION_ERROR -> FAILED/ERROR`. El mapping anterior que convertia ambiguedad en `FAILED/ERROR` recuperable quedo supersedido por 4D-1B.
- Fiscal retry backend-only vigente: `ElectronicDocumentUseCase.retrySend(Long)` permanece como contrato interno, pero 4D-1B lo deja fail-closed y sin llamada al provider hasta que exista consulta/reconciliacion remota capaz de demostrar ausencia del comprobante. Todo intento directo registra `BLOCKED` en una transaccion local corta cuando aplica; no consume correlativo, no regenera XML, no refirma y no existe endpoint REST, scheduler, backoff, polling ni retry automatico.
- Fiscal frontend-readiness contract documental implementado: 3B-4 crea el contrato para futura UI/API fiscal sin tocar frontend ni backend funcional; se documentan matriz de estados, acciones futuras, reglas de retry readiness, mensajes operativos, roles, auditoria visible, datos prohibidos y draft API para `fiscal-readiness`/`attempts` con `retry-send` diferido. Validacion documental: estado Git verificado, no se modifico codigo funcional, no se implemento endpoint, no se crearon migraciones. `retry-send` queda diferido hasta fase futura de exposicion API/UI.
- Fiscal evidence metadata DB foundation completado en 3C-1: V24 crea `electronic_document_evidence` como inventario append-only de metadata segura; se agregan modelo/enums/puerto/entidad/mapper/adapter para evidencias `SIGNED_XML`, `CDR`, `PDF`, `TICKET`, `QR` y `PROVIDER_RESPONSE_METADATA`, sin guardar payloads completos ni storage real. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (109 tests), `./mvnw test` OK (551 tests). No se reemplaza `billing_xml_files`, no se modifican `send`, `retrySend` ni `sign`, no se crea endpoint REST y no se toca frontend.
- Fiscal evidence write/read contract interno completado en 3C-2: `sign`, `send` y `retrySend` registran metadata segura `SIGNED_XML`/`PROVIDER_RESPONSE_METADATA` sin payloads completos; `SIGNED_XML` referencia `billing_xml_files` via `DB_LEGACY` y hashes SHA-256; provider response metadata queda ligada al attempt `SEND` cuando aplica, con status/ticket/correlation id sanitizados y `storageProvider=NONE`; `ElectronicDocumentUseCase.evidence(Long id)` agrega lectura interna por documento. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (111 tests), `./mvnw test` OK (553 tests). No se creo endpoint REST, no se toco frontend, no se implemento storage real, descargas, PSE/OSE/SUNAT real, CDR/PDF/QR reales ni retry automatico.
- Fiscal evidence readiness/API draft update completado en 3C-3: contrato documental cierra el futuro `evidenceSummary` metadata-only para UI/API read-only, con banderas seguras (`hasSignedXml`, `hasProviderMetadata`, `evidenceCount`, `lastEvidenceAt`, `evidenceTypes`, `lastProviderStatus`, `lastProviderTicketMasked`, `lastProviderCorrelationIdMasked`, `simulated`, `storageProviderSummary`, `availableActions`, `operatorSeverity`, `displayMessage`). No se implemento endpoint REST, no se toco frontend, no se agrego storage real ni se expusieron payloads o secretos. 3B-3B sigue diferida y 3C-4 storage real sigue diferida.
- Fiscal evidence storage architecture 3C-4A completado documentalmente: se crearon ADR, threat model, politica preliminar de retencion/cifrado/auditoria, runbook preliminar backup/restore y QA documental para storage fiscal futuro. Decision: storage real sigue diferido; antes de cualquier filesystem/S3/GCS se requiere resolver proveedor, cifrado, retencion/legal hold, auditoria, roles y migracion desde `DB_LEGACY`. Subfases documentadas: 3C-4B modelo/migracion avanzada, 3C-4C puerto + adapters no productivos, 3C-4D filesystem LOCAL/BETA opcional, 3C-4E S3/GCS PROD futuro, 3C-4F descarga/API/auditoria. No se modifico codigo, no se crearon migraciones, no se creo endpoint REST, no se toco frontend, no se implemento storage real ni adapters. 3B-3B sigue diferida.
- Fiscal evidence storage seam 3C-4C completado: se agrego `FiscalEvidenceStoragePort` con `store`, `exists`, `verifyChecksum` y `metadataOnly`, sin `openRead`; value objects metadata-only y validacion defensiva contra payloads, rutas, secretos, certificados y claves cloud sensibles; adapters no productivos `NoopFiscalEvidenceStorageAdapter` (`NONE`) y `LegacyBillingXmlEvidenceStorageAdapter` (`DB_LEGACY` + `SIGNED_XML`) sin escritura real ni exposicion de XML legacy. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (118 tests) y `./mvnw test` OK (560 tests). No se creo V25, no se modifico V24, no se integro con `sign`, `send` ni `retrySend`, no se creo endpoint REST, no se toco frontend, no se creo storage real ni adapters filesystem/S3/GCS. 3B-3B sigue diferida.
- Fiscal evidence filesystem synthetic adapter 3C-4D-1 completado: se agrego `FiscalEvidenceFilesystemStoreCommand` para contenido sintetico controlado y `FilesystemFiscalEvidenceStorageAdapter` no productivo para `FILESYSTEM` solo LOCAL/BETA, deshabilitado por defecto y con base dir explicita. Implementa path normalization, bloqueo de PROD, no overwrite/put-if-absent, checksum SHA-256, size check y cleanup de temporales. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (129 tests) y `./mvnw test` OK (571 tests). No existe `openRead`, descarga, endpoint REST, access audit, V25, S3/GCS ni integracion con `sign`, `send` o `retrySend`; no se guardan XML/CDR/PDF/QR reales y 3B-3B sigue diferida.
- Fiscal evidence readiness API 3D-A implementado backend-only: `GET /api/v1/billing/documents/{documentId}/evidence-readiness` expone disponibilidad provider-agnostic para `ADMIN`, `SUPERVISOR` y `CAJERO`; bloquea `ALMACENERO` y requests sin autenticar mediante seguridad existente. `downloadAllowed=false`, `allowedActions=[]`; `REGISTERED` se calcula conservadoramente como `NOT_READY`; checksum/size no implican integridad verificada; no se inventa `CORRUPTED`. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (132 tests), `./mvnw -Dtest=BillingEvidenceReadinessIntegrationTest test` OK (8 tests) y `./mvnw test` OK (582 tests). Sin contenido fiscal, storage keys/paths/providers, V25, access audit, descarga, AWS, frontend ni cambios a `sign`, `send`, `retrySend` o endpoint XML legacy. 3C-4E, 3C-4F Build y 3B-3B siguen diferidas.
- MiFact JSON baseline 4C documentado: ADR `CORE-ADR-004`, baseline publico trazable, borrador provider-neutral y siete fixtures JSON sanitizados para solicitudes, estados `101`/`102`/`103`/`104` y metadata de evidencia. MiFact JSON queda seleccionado solo como candidato futuro; sin cliente HTTP, llamadas demo, secretos, codigo Java, migraciones ni integracion con `send`/`retrySend`. Se documentan query-before-retry, ownership InkToy de correlativos, token en frontera de transporte, evidencia Base64 conservadora y la secuencia 4D antes de cualquier cliente HTTP. 3B-3B sigue diferida.
- Fiscal locking reproduction 4D-1A completada con PASS: un test PostgreSQL/Testcontainers reproduce el conflicto entre `FOR UPDATE` sobre `electronic_documents` y el INSERT independiente de un attempt cuya FK solicita `FOR KEY SHARE` sobre la misma fila. `pg_blocking_pids` identifica la conexion A como blocker de B; PostgreSQL reporta `wait_event_type=Lock`, `wait_event=transactionid`, `PSQLException` y SQLSTATE `55P03` tras `lock_timeout` local de 1 segundo. La prueba se ejecuto 9 veces de forma estable y dejo 0 attempts persistidos. Validaciones relacionadas: 12 tests de integracion billing PASS y `BillingApplicationServiceTest` PASS (132 tests). No se modifico codigo funcional, servicios, repositories, migraciones, configuracion, frontend ni `.env`; no se inicio 4D-1B ni se realizaron llamadas MiFact.
- Fiscal submission transaction boundary hardening 4D-1B completado y publicado: `send()` ahora orquesta sin transaccion exterior `prepare/commit -> provider -> finalize`; `FiscalSendTransactionService` concentra fases `REQUIRED` cortas y los writes de attempts usan `MANDATORY`. El provider se invoca sin transaccion Spring activa ni lock PostgreSQL retenido; otra conexion observa `SENT + STARTED` durable y puede adquirir `FOR UPDATE NOWAIT` mientras el provider espera. Un lock de documento serializa preparaciones; solo un attempt queda en vuelo y solo una llamada provider se autoriza por documento. Finalizaciones identicas son idempotentes y las contradictorias se rechazan; timeout, indisponibilidad, comunicacion y excepciones quedan `SENT/PENDING`, no recuperables por reenvio directo; fallo local tras respuesta remota conserva `SENT/STARTED` reconciliable. `retrySend()` queda fail-closed. Validaciones: focal 4D-1B 16 tests PASS; integraciones billing relacionadas 28 PASS; `BillingApplicationServiceTest` 132 PASS; suite backend completa 601 PASS. 4D-1A permanece intacta y PASS (3 tests, SQLSTATE `55P03`). Commit publicado: `cd9cd21 fix(billing): harden fiscal send transaction boundaries`. Sin REST, frontend, migraciones, V25, MiFact real, query/reconcile, `.env`, secretos ni infraestructura.
- Fiscal series administrative row-lock alignment 4D-2A cerrado y publicado en `9976ec5 fix(billing): lock fiscal series administrative updates`: `BillingSeriesApplicationService.update()`, reactivacion mediante `update(active=true)` y `deactivate()` adquieren desde el inicio el mismo `findByIdForUpdate()`/`PESSIMISTIC_WRITE` que usa `createFromSale()`. Las validaciones de identidad, serie activa y maximo emitido operan sobre estado fresco bloqueado; una emision confirmada no puede ser sobrescrita por un payload administrativo obsoleto y el orden se mantiene `serie -> escrituras dependientes`. `FiscalSeriesConcurrencyIntegrationTest` valida PostgreSQL real con sesiones independientes, `pg_blocking_pids`, colas recursivas de waiters, rollback y timeouts acotados. Cierre publicado: focal 4D-2A 8 PASS; conjunto billing/4D 170 PASS; `BillingApplicationServiceTest` 134 PASS; suite backend completa 611 PASS. 4D-2B y 4D-2C permanecian diferidas en ese cierre.
- Additive optimistic-concurrency foundation 4D-2B-1 implementada localmente: V25 agrega `billing_series.version BIGINT NOT NULL DEFAULT 0` con check no negativo; `BillingSeriesEntity` usa `@Version`; responses incluyen `version`; POST/GET por ID/PUT/DELETE emiten ETag fuerte `"billing-series-{id}-v{version}"`; PUT y DELETE aceptan `If-Match` opcional. Precondicion invalida devuelve `400`, token stale devuelve `412` con rollback y sin exponer ORM; ausencia del header sigue compatible. La comparacion ocurre despues de `findByIdForUpdate()`, `saveAndFlush()` devuelve la version confirmada y una emision que modifica `currentNumber` invalida tokens previos. CORS permite `If-Match` y expone `ETag`. Focales: parser 4 PASS; migracion V24/V25/from-zero 1 PASS; REST/concurrencia/rollback 14 PASS; handler 6 PASS; CORS 2 PASS; `BillingApplicationServiceTest` 135 PASS; `FiscalSeriesConcurrencyIntegrationTest` 9 PASS. Regresion fiscal combinada: 188 PASS; integraciones billing: 24 PASS; suite backend completa: 634 PASS. B2 (Angular) y B3 (`If-Match` obligatorio/428) siguen diferidas; no mezclar backends pre/post-`@Version` durante rollout.
