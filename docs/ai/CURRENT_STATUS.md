...
- Fiscal attempts send audit base completado: V23 crea `electronic_document_attempts` no destructiva; se agregaron `ElectronicDocumentAttempt`, `FiscalOperation`, `FiscalAttemptResult`, `FiscalErrorCategory`, puerto/repositorio/mapper/adapter JPA y `FiscalAttemptAuditService`; `send` registra attempts `STARTED`, `SUCCESS`, `FAILED` o `BLOCKED` para operacion SEND, con `attemptNumber`, hashes SHA-256, provider status/ticket/message sanitizados, `simulated=true` en LOCAL/BETA y sin retry automatico; excepciones del provider se auditan como `FAILED` recuperable con transaccion separada y se relanzan. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (72 tests), `./mvnw test` OK (513 tests). No se toco frontend; no se implemento scheduler, backoff, reenvio real, secret manager real, SUNAT/PSE/OSE, firma real, CDR, XML UBL SUNAT completo, PDF/ticket fiscal, QR, notas ni bajas; no se tocaron `.env`, secretos ni certificados reales.
- Fiscal provider response mapping completado: `ProviderSendResult` ahora conserva compatibilidad y agrega `ProviderSendStatus`, provider code, correlation id, `FiscalErrorCategory`, flags recoverable/observed/pending/simulated y factories seguras; `FiscalProviderResultClassifier` centraliza el mapping a `FiscalAttemptResult`, `FiscalErrorCategory`, recuperabilidad y estado fiscal final. Mapping actual: ACCEPTED -> SUCCESS/ACCEPTED; OBSERVED -> SUCCESS/ACCEPTED + `PROVIDER_OBSERVED`; REJECTED -> FAILED/REJECTED; PENDING -> attempt `PENDING` y documento `SENT` sin transicion final ni reenvio; TIMEOUT/UNAVAILABLE/COMMUNICATION_ERROR -> FAILED/ERROR recuperable sin retry automatico; CONFIGURATION_ERROR -> FAILED/ERROR no recuperable. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (79 tests), `./mvnw test` OK (520 tests). No se toco frontend ni DB/Flyway; no se implemento retry automatico, polling, scheduler, backoff, PSE/OSE/SUNAT real, firma real, CDR, XML UBL SUNAT completo, PDF/ticket fiscal, QR, notas ni produccion real; no se tocaron `.env`, secretos ni certificados reales.
- Fiscal manual retry policy backend-only implementado: `ElectronicDocumentUseCase.retrySend(Long)` y `ElectronicDocumentApplicationService.retrySend(Long)` agregan retry manual/controlado separado de `send`; usa lock `findByIdForUpdate`, valida documento `ERROR`, ultimo attempt `SEND` `FAILED`, `recoverable=true`, categoria permitida (`PROVIDER_TIMEOUT`, `PROVIDER_UNAVAILABLE`, `COMMUNICATION_ERROR`) y XML firmado existente; crea nuevo attempt `SEND` incremental, registra `STARTED` antes del provider y finaliza via `FiscalProviderResultClassifier`; bloqueos registran `BLOCKED` con sanitizacion existente. `PENDING` queda bloqueado en `SENT`; `OBSERVED` queda `ACCEPTED` y no reintentable. Validaciones: `./mvnw -Dtest=BillingApplicationServiceTest test` OK (96 tests), `./mvnw test` OK (537 tests). No se toco frontend ni DB/Flyway; no se creo endpoint REST; no se implemento retry automatico, scheduler, backoff, polling, PSE/OSE/SUNAT real, firma real, CDR, XML UBL SUNAT completo, PDF/ticket fiscal, QR, notas ni produccion real; no se tocaron `.env`, secretos ni certificados reales.
