...
| QA-008 | contracts/pagination | MEDIUM    | Persisten warnings de serializacion `PageImpl` en endpoints `v1` legacy (`products`, `inventory/stocks`).                                | Contrato `v1` aun expone `Page<T>` por compatibilidad temporal mientras frontend migra.     | `backend/src/main/java/com/erppos/backend/erp/catalog/adapter/rest/ProductController.java`, `backend/src/main/java/com/erppos/backend/erp/inventory/adapter/rest/InventoryController.java`, `backend/src/main/java/com/erppos/backend/erp/catalog/adapter/rest/ProductV2Controller.java`, `backend/src/main/java/com/erppos/backend/erp/inventory/adapter/rest/InventoryV2Controller.java` | BT-006 Fase 1 agrega `v2` con DTO estable (`items/page/size/totalItems/totalPages`) y mantiene `v1` como legado temporal para no romper frontend. | Mitigado | `chore(api): migrate frontend consumers from v1 page to v2 stable` |
