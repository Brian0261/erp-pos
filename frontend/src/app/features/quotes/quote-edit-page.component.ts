      ...
      <header class="ui-page-head">
        <div>
          ...
          <p class="ui-page-description">
            Actualiza solo cotizaciones en estado borrador.
          </p>
        </div>
        <a
        ...
        <section class="form-section">
          ...
          <div class="items-table" formArrayName="items">
            ...
            >
              ...
              <div class="items-table__cell items-table__cell--action" data-label="Accion">
...
export class QuoteEditPageComponent implements OnInit {
  ...
  private patchForm(quote: QuoteResponse): void {
    ...
    for (let i = 0; i < quote.items.length; i++) {
      ...
      if (product) {
        ...
        this.selectedProducts.push(null);
        this.resolveProductForItem(i, item.productId);
      }
    }
  ...
  private resolveProductForItem(index: number, productId: number): void {
    this.productService.getById(productId).subscribe({
      next: (product) => {
        this.productById.set(product.id, product);
        const group = this.items.at(index);
        const currentProductId = this.normalizeNumber(
          group?.get("productId")?.value,
        );
      ...
      error: () => {
        // Silently ignore; user can still see product was selected
      },
    });
  }
